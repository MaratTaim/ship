 ...
 *
 */
public class BigPier extends BasePier implements Runnable{
    ...
    public boolean unloading(Ship ship) {
        try {
            if (place1.tryAcquire(5, TimeUnit.MILLISECONDS)) {
                System.out.println("Большой Пирс: Мы начали разгрузку корабля - " + ship.name() + " !");
                Thread.sleep(timeWork+ship.volume());
                place1.release();
                System.out.println("Большой Пирс: Мы разгрузили "+ ship.name());
                return true;
            }else if (place2.tryAcquire(5, TimeUnit.MILLISECONDS)){
                System.out.println("Большой Пирс: Уы нашали лазгрузку коабля - " + ship.name() + " !");
                Thread.sleep(timeWork + ALCOHOLIC + ship.volume());
                place2.release();
                System.out.println("Большой Пирс: Всё разглузили "+ ship.name());
                return true;
            }else{
                return false;
            }
